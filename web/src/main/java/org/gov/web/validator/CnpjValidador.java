...
public class CnpjValidador implements Validator {
	...
	public CnpjValidador() {
		...
		resourceBundle = ResourceBundle.getBundle("application-error-message", new Locale("pt", "BR"));
	...
	public void validate(FacesContext facescontext, UIComponent uicomponent, Object obj) {
		...
		cnpj = cnpj.replaceAll(Pattern.quote("."), "").replaceAll("/", "").replace("_", "").replace("-", "");
		...
		if (cnpj.length() == NUMBER_QUATORZE) {
			...
			try {
				if(!CNPJUtil.isValid(cnpj)) {
					throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "CNPJ Incorreto", null
							));
				}

			} catch (IllegalArgumentException e) {
				throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro na aplicação", null
						));
			}
